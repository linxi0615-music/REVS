...
load_communities_data <- function(file_path) {
  tryCatch({
    # 读取数据
    data <- read.csv(file_path, header = FALSE, na.strings = c("", "NA", "?"))
    cat("数据集形状:", dim(data), "\n")
...
run_single_experiment <- function(X, Y) {
  ...
  methods <- c("REVS-BART","REVS-RF", "BART","TVS-RF", "TVS-BART", "DART", "Lasso", "SSLASSO", "SIS")
...
run_multiple_experiments <- function(X, Y, n_runs = 3) {
  ...
  for (i in 1:n_runs) {
    set.seed(2025+n_runs)#123 125 250 2025 2026
    cat(paste("\n================ 第", i, "次实验 ================\n"))
